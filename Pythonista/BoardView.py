...
class ColoredLayer(scene.ShapeNode):  # scene.Layer):
    ...
    def __init__(self, frame, parent, fill_color=colorDefault, *args, **kwargs):
        x, y, w, h = frame
        super().__init__(ui.Path.rect(0, 0, w, h), anchor_point=(0, 0),
                         fill_color=fill_color, parent=parent, position=(x, y),
                         stroke_color='gold', *args, **kwargs)
        print('{} {} frame: {}, {}, {}, {}'.format(self.__class__.__name__, fill_color, x, y, w, h))
...
class PegboardView(ColoredLayer):
    ...
    def square_rect(self, x, y):
        ss = self.square_size
        return self.inset_rect(scene.Rect(self.frame.x + x * ss,
                                          self.frame.y + y * ss, ss, ss))
...
class PlayerHandView(ColoredLayer):
    ...
    def syncView(self):
        ...
        for i in range(dominoCount):
            ...
            #dominoLayer.frame.x = pixelsBetweenDominos + i * (self.dominoBoardView.dieWidth + pixelsBetweenDominos)
...
class BoneyardView(ColoredLayer):
    ...
    def syncView(self):
        ...
        for i in range(dominoCount):
            ...
            dominoLayer = self.scene.getDominoView(d)
            #dominoLayer.frame.center(self.frame.center())
            #dominoLayer.frame.y = self.frame.y + pixelsBetweenDominos + i * (dieWidth + pixelsBetweenDominos)
...
class DominoBoardView(scene.Scene):
    ...
    def makeDictOfDominoViews(self):
        ...
        for theDomino in self.dominoWorld.mDominos:
            domino = Domino(pips=theDomino, die_size=self.dieWidth,
                            parent=self.humanView)
    ...
    def touch_ended(self, touch):
        if not self.touchedLayer:
            return
        if not touch.location in self.playAreaView.frame:
