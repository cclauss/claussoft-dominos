...
class Domino(scene.SpriteNode):
    def __init__(self, pips=(5, 6), die_size=70, fg_color='blue', bg_color='silver',
                 *args, **kwargs):
        assert len(pips) == 2, 'Need two pips!'
        image = domino_image(pips=pips, die_size=die_size, fg_color=fg_color,
                             bg_color=bg_color)
        #print(image, pips)
...
if __name__ == '__main__':
    class OneDominoScene(scene.Scene):
        def setup(self):
            ...
            print(die_size)
            domino = Domino(pips=(5, 6), die_size=die_size, fg_color='pink',
                            bg_color='green', parent=self,
                            position=self.bounds.center())
            domino.anchor_point=(.5, .5)
