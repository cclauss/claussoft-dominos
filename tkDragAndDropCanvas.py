...
def coordInBbox(
    inCoord=(10, 10), inBbox=(0, 0, 20, 20)
...
class tkDraggableWidget(object):
    def __init__(self, inDNDFrame, inWidget, inName):
        ...
        if not self.mWidget:
            theCanvas = self.mDNDFrame.getCanvas()
            self.mWidget = theCanvas.create_oval(
                150, 200, 200, 250, outline="blue", fill="lightgray", tags=self.mName
            )
...
class tkDragAndDropFrame(ttk.Frame):
    def __init__(self, master=None, width=1280):
        ...
        self.mCanvas.grid(column=0, row=0, columnspan=10, rowspan=10, sticky=NSEW)
    ...
    def whichDropZone(self, inCoord):
        for dropZoneAndName in self.mDropZones:
            print("Checking zone:", dropZoneAndName[1], self.bbox(dropZoneAndName[0]))
...
class SampleApp(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.theWidgets = [
            tkDraggableWidget(self.mDNDFrame, None, "Widget 1"),
